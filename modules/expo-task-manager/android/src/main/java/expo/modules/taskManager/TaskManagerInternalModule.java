...
public class TaskManagerInternalModule implements InternalModule, ModuleRegistryConsumer, TaskManagerInterface {
  ...
  public void setModuleRegistry(ModuleRegistry moduleRegistry) {
    ...
    mTaskService.setTaskManager(this, getAppId());
  }
  ...
  public void executeTaskWithBody(Bundle body) {
    Log.i("EXPO", "TaskManagerModule.executeTaskWithBody");
    if (mEventsQueue != null) {
      // `startObserving` wasn't called yet - add event body to the queue.
      mEventsQueue.add(body);
    } else {
      Log.i("EXPO", "Sending event with body " + body.toString());
